...
{
  (void)env, (void)clazz;
  // struct btck_Chain* ptr = btck_Chain_New();
...
JNIEXPORT jint JNICALL Java_Chain_size(JNIEnv* env, jobject self)
{
  ...
  return (jint)btck_Chain_GetSize(ptr);
}
...
JNIEXPORT jobject JNICALL Java_Chain_get(JNIEnv* env, jobject self, jint idx)
{
  ...
  struct btck_Block* blk = btck_Chain_At(ptr, (size_t)idx);
  if (blk == NULL) {
    return NULL;
  }
  ...
  jobject blkObj = (*env)->NewObject(env, blkClass, ctor, (jlong)(uintptr_t)blk);
  return blkObj;
}
