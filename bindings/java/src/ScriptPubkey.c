...
Java_ScriptPubkey_construct(JNIEnv* env, jclass clazz, jbyteArray raw)
{
  (void)clazz;
...
Java_ScriptPubkey_equal(JNIEnv* env, jobject self, jobject other)
{
  jfieldID fid = get_native_handle_field(env, self);
  jlong handle1 = (*env)->GetLongField(env, self, fid);
  jlong handle2 = (*env)->GetLongField(env, other, fid);
  struct BtcK_ScriptPubkey* ptr1 =
    (struct BtcK_ScriptPubkey*)(uintptr_t)handle1;
  struct BtcK_ScriptPubkey* ptr2 =
    (struct BtcK_ScriptPubkey*)(uintptr_t)handle2;
  if (!ptr1 || !ptr2) {
    return JNI_FALSE;
  }
