...
static PyObject* new(PyTypeObject* Py_UNUSED(type), PyObject* args, PyObject* kwargs)
{
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "LO!", kwlist, &amount, ScriptPubkey_Type, &script_pubkey)) {
    return NULL;
  }

  return TransactionOutput_New(btck_TransactionOutput_New(amount, ScriptPubkey_GetImpl(script_pubkey)));
}
...
static PyObject* get_script_pubkey(struct Self const* self, void* Py_UNUSED(closure))
{
