# Copyright (c) 2025-present The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

import btck
import pytest

def test_script_verify():
    assert btck.VerificationFlags.ALL|btck.VerificationFlags.NONE == btck.VerificationFlags.ALL


def test_script1():
    # a random old-style transaction from the blockchain
    sp = btck.ScriptPubkey(bytes.fromhex("76a9144bfbaf6afb76cc5771bc6404810d1cc041a6933988ac"))
    assert sp.verify(
        amount = 0,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script2():
    # a random segwit transaction from the blockchain using P2SH
    sp = btck.ScriptPubkey(bytes.fromhex("a91434c06f8c87e355e123bdc6dda4ffabc64b6989ef87"))
    assert sp.verify(
        amount = 1900000,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script3():
    # a random segwit transaction from the blockchain using native segwit
    sp = btck.ScriptPubkey(bytes.fromhex("0020701a8d401c84fb13e6baf169d59684e17abd9fa216c8cc5b9fc63d622ff8c58d"))
    assert sp.verify(
        amount = 18393430,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script4():
    # a random old-style transaction from the blockchain - WITH WRONG SIGNATURE for the address
    sp = btck.ScriptPubkey(bytes.fromhex("76a9144bfbaf6afb76cc5771bc6404810d1cc041a6933988ff"))
    assert not sp.verify(
        amount = 0,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script5():
    # a random segwit transaction from the blockchain using P2SH - WITH WRONG AMOUNT
    sp = btck.ScriptPubkey(bytes.fromhex("a91434c06f8c87e355e123bdc6dda4ffabc64b6989ef87"))
    assert not sp.verify(
        amount = 900000,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script6():
    # a random segwit transaction from the blockchain using native segwit - WITH WRONG SEGWIT
    sp = btck.ScriptPubkey(bytes.fromhex("0020701a8d401c84fb13e6baf169d59684e17abd9fa216c8cc5b9fc63d622ff8c58f"))
    assert not sp.verify(
        amount = 18393430,
        tx_to = btck.Transaction(bytes.fromhex("")),
        flags = btck.VerificationFlags.ALL & ~btck.VerificationFlags.TAPROOT
    )


def test_script7():
    # `TAPROOT` without providing `spent_outputs`
    sp = btck.ScriptPubkey(bytes.fromhex("76a9144bfbaf6afb76cc5771bc6404810d1cc041a6933988ac"))
    with pytest.raises(btck.VerificationError) as err:
        sp.verify(
            amount = 0,
            tx_to = btck.Transaction(bytes.fromhex("")),
            flags = btck.VerificationFlags.ALL
        )

    assert str(err.value) == "The taproot flag was set, so valid spent_outputs have to be provided."
